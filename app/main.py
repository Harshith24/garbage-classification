...
from yolo_helpers import get_yolo_net, yolo_forward, yolo_save_img
...
weights_path = os.path.join('yolo', 'yolov4-11-classes_final.weights')
...
@app.route('/BehindThe-Scene.html')
# @app.route(base_url + '/BehindThe-Scene.html')
def BTS():
    return render_template('BehindThe-Scene.html')

@app.route('/About-the-Team.html')
# @app.route(base_url + '/About-the-Team.html')
def team():
...
#         flash('No file part')
    ...
    # if user does not select file or selects an invalid file,
    # browser also submit an empty part without filename
    if file.filename in [".png", ".jpg", ".jpeg"] or file.filename == "" \
        or file.filename.split(".") in [".tiff", ".gif"]:
...
def results(filename): 
    ...
    if len(class_ids) > 0:
        ...
        return render_template('Results.html', labels='No Garbage Detected', old_filename=filename, filename=filename)
